# ============================================
# IGORBARBO V8 ULTIMATE - VERSÃO CORRIGIDA (MONOLÍTICA)
# ============================================
...
def analisar_preco_ativo(ticker, dados_historicos):
    if not dados_historicos:
        return "neutro", "🔵 DADOS INSUFICIENTES", "#808080", "Não há dados históricos suficientes para análise.", 0
...
def criar_authenticator():
    ...
    COOKIE_KEY = "chave_super_secreta_123_mude_isso"
...
# Criar usuário admin se não existir (executar apenas uma vez)
def criar_usuario_admin():
    c = conn.cursor()
    c.execute("SELECT * FROM usuarios WHERE username = 'admin'")
    if not c.fetchone():
        hashed = stauth.Hasher(['1234']).generate()[0]
        c.execute("INSERT INTO usuarios (username, nome, senha_hash) VALUES (?, ?, ?)",
                  ("admin", "Administrador", hashed))
        conn.commit()
        st.success("Usuário admin criado com senha 1234")
    else:
        st.info("Usuário admin já existe.")

# Descomente a linha abaixo se quiser criar o admin automaticamente na primeira execução
# criar_usuario_admin()# ============================================
# SISTEMA DE LOGIN
# ============================================
        ...
        for ticker in df['ticker']:
            ...
            if preco_teto and preco_atual:
                ...
                if preco_atual <= preco_teto:
                    ...
                    status = "⏳ ESPERAR"
                    cor = "#FFA500"
                resultados_teto.append({'Ticker': ticker, 'Preço Atual': preco_atual, 'Preço Teto': preco_teto, 'Diferença %': diferenca, 'Status': status})
        if resultados_teto:
            df_teto = pd.DataFrame(resultados_teto)
            st.dataframe(df_teto.style.format({'Preço Atual': 'R$ {:.2f}', 'Preço Teto': 'R$ {:.2f}', 'Diferença %': '{:.1f}%'}), use_container_width=True)
    ...
    if df.empty:
        ...
        with tab_av2:
            ...
            with st.spinner("Calculando métricas de risco..."):
                ...
                if dados_risco:
                    ...
                    fig = px.scatter(df_risco, x='Volatilidade %', y='Retorno Anual %', text=df_risco.index, title="Risco x Retorno", labels={'Volatilidade %': 'Risco (Volatilidade)', 'Retorno Anual %': 'Retorno Esperado'})
        ...
        with tab_av4:
            ...
            with col_exp1:
                ...
                output.seek(0)
                st.download_button(label="📥 Baixar Excel", data=output, file_name=f"carteira_{datetime.now().strftime('%Y%m%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            with col_exp2:
                st.write("**Exportar para CSV**")
                csv_data = df_export.to_csv(index=False).encode('utf-8')
                st.download_button(label="📥 Baixar CSV", data=csv_data, file_name=f"carteira_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv", use_container_width=True)
                ...
                with st.container():
                    ...
                    if st.session_state.editando == row['ticker']:
                        with st.form(key=f"form_edit_{row['ticker']}"):
                            ...
                            novo_setor = st.selectbox("Setor", ["Ações", "FII Papel", "FII Tijolo", "ETF", "Renda Fixa"], index=["Ações", "FII Papel", "FII Tijolo", "ETF", "Renda Fixa"].index(row['setor']))
