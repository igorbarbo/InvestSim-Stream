...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS alertas
                 (id TEXT PRIMARY KEY, ticker TEXT, tipo TEXT, preco REAL, ativo BOOL, criado_em TEXT)''')
    conn.commit()
    return conn
...
def salvar_alerta(ticker, tipo, preco):
    """Salva alerta no banco"""
    try:
        c = conn.cursor()
        alerta_id = f"{ticker}_{tipo}_{preco}_{datetime.now().timestamp()}"
        c.execute("INSERT INTO alertas VALUES (?, ?, ?, ?, ?, ?)", 
                  (alerta_id, ticker, tipo, preco, 1, datetime.now().strftime('%d/%m/%Y %H:%M')))
        conn.commit()
        return True
    except Exception as e:
...
def carregar_alertas():
    ...
    try:
        c = conn.cursor()
        c.execute("SELECT id, ticker, tipo, preco, ativo, criado_em FROM alertas WHERE ativo = 1")
        resultados = c.fetchall()
        alertas = {}
        for r in resultados:
            alertas[r[0]] = {
...
if 'alertas' not in st.session_state:
    st.session_state.alertas = carregar_alertas()
...
def pegar_preco_simples(ticker):
    """Versão simples que só retorna o preço"""
...
def calcular_retorno_esperado(alocacao):
    ...
    if df.empty:
        ...
        with tab_alerta1:
            ...
            if st.button("✅ Ativar Alerta", use_container_width=True):
                if salvar_alerta(ticker_alerta, tipo_alerta, preco_alerta):
                    st.session_state.alertas = carregar_alertas()
        ...
        with tab_alerta2:
            if not st.session_state.alertas:
                ...
                for alerta_id, alerta in list(st.session_state.alertas.items()):
                        ...
                        with st.form(key=f"form_edit_{row['ticker']}"):
                            ...
                            with col_s1:
                                if st.form_submit_button("Salvar"):
                                    atualizar_ativo(row['ticker'], nova_qtd, novo_pm, novo_setor)
                                    st.session_state.editando = None
                                    st.rerun()
                            with col_s2:
